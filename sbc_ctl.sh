...
EOF
    ...
    docker run -d \
        ...
        --cap-add=SYS_PTRACE \
        --network=host \
        -v $data_path/sbc:/var/lib/fonasip/sbc \
...
status() {
    ...
    if [ -z "$service_name" ]; then
        ...
        echo ""
        docker exec fonasip.sbc supervisorctl status 
    else
        echo ""
        echo "status service $service_name"
        echo ""
        docker exec fonasip.sbc supervisorctl status $service_name
    fi
}
...
restart() {
    ...
    if [ -z "$service_name" ]; then
        ...
        echo ""
        docker restart -t 300 fonasip.sbc
        exit
    fi
    ...
    echo ""
    docker exec fonasip.sbc supervisorctl restart $service_name
}
...
start() {
    ...
    if [ -z "$service_name" ]; then
        ...
        echo ""
        docker start fonasip.sbc
    else
        echo ""
        echo "start service $service_name"
        echo ""
        docker exec fonasip.sbc supervisorctl start $service_name
    fi
}
...
stop() {
    ...
    if [ -z "$service_name" ]; then
        ...
        echo ""
        docker stop -t 300 fonasip.sbc
        exit
    fi
    echo ""
    echo "stop service $service_name"
    echo ""
    docker exec fonasip.sbc supervisorctl stop $service_name
}
